...
{
    public const string DEFAULT_COMMAND_TAG = 'console.command';
    ...
    private function configureHttpRequest(
        ...
        try {
            ...
            throw new InvalidState(
                "Service of type '$factoryClass' not found. Option '$this->name > http > override' is enabled. Register extension '$httpExtensionClass' or '$factoryClass' service."
            );
        }
        ...
        foreach ($httpConfig->headers as $name => $value) {
            ...
            if ($name !== $lowerName) {
                throw new InvalidArgument(
                    "Only lowercase header names are supported. Incorrect case of config key '$this->name > http > headers > $name'. Use '$lowerName' instead of '$name'."
                );
            }
